...
def subjects():
    ...
    if tableName is None or tableName == '':
        mainLabel1.config(text='You Need to select a table.', font=14, bg=color['TABLE_NOT_FND_COLOR'], fg='red')
...
def database():
    ...
    Label(New_Table_Window, text='  New table name: ', font=(text['TEXT_FONT'], 14), bg=color['MENU_COLORS']).grid(row=1, column=0)
    ...
    def ok():
        z = [temp.lower() for temp in listdir('Database') if temp.endswith('.db')]
        ...
        if len(En.get()) > 16:
            ...
            lb1.config(text=' ❌ Give the table a name', font=(text['TEXT_FONT'], 14), bg=color['MENU_COLORS'], fg=color["ERROR_COLOR"])

        lb1.grid(row=3, column=1, pady=10)
        lb1.after(4000, lambda: lb1.grid_forget())

    bt = Button(New_Table_Window, text=' Ok ', command=ok, font=(text['TEXT_FONT'], 9))
...
def slt_table():
    ...
    if len(out) > 0:
        selectWindow.geometry('400x300')
        Label(selectWindow, text='  Select any one of the table ', font=15, bg=color['MENU_COLORS'], fg=color['DATABASE_TXT_COLOR']).grid(row=0, column=0)
        radio = IntVar()

        def selection():
            fileSettings['table_name'] = getcwd() + './Database/' + out[radio.get() - 1] + '.db'
            saveSettings(fileSettings)
            fName = loadSettings()['table_name'].split('/')[-1].replace('.db', '')
            mainLabel1.config(text=f' Log  ~  Table Name - {str(fName).ljust(20)}', font=(text['MAIN_HEADING_FONT'], 15, 'bold'),
                              bg=color['BACKGROUND_COLOR'], fg=color['TABLE_TITLE_COLOR'])
        ...
        scroll = Scrollbar(selectWindow, orient='vertical', command=canvas.yview, bg=color['MENU_COLORS'])
        r, z = 3, 1
...
def displaySubject():
    ...
    if len(n_s) > 0:
        tp4.config(text='  These are the subject  ', bg=color["CHILD_COLOR"], font=(text["SUB_HEADING_FONT"], 15), justify='center')
        disSubList.delete('0', 'end')
        disSubList.config(width=50, font=(text["TEXT_FONT"], 14), bg=color["CHILD_COLOR"])
        ...
        tp2.config(text='   !! No Subject were found !!     ', font=(text["ALERT_FONT"], 15, 'bold'), fg=color["ERROR_COLOR"], bg=color["CHILD_COLOR"])
        tp2.grid(row=0, column=0)

    disSubBtn.config(text='X'.center(3), command=close, bg='red')
    disSubBtn.grid(row=0, column=2, sticky=NE, ipadx=10, pady=5, ipady=2) # row=0, column=2, sticky=NE, ipadx=15, pady=5
...
def newSubject():
    ...
    tp2.config(text=' Add New Subject', font=(text["SUB_HEADING_FONT"], 20, 'bold'), justify='center', bg=color["CHILD_COLOR"], fg=color["TEXT_COLOR"])
    tp2.grid(row=0, column=0, ipady=15)
    nslb1.config(text='Enter the new Subject Name', font=12, bg=color["CHILD_COLOR"])
    nslb2.config(text='Enter the Subject Shortcut Key', font=12, bg=color["CHILD_COLOR"])
    ...
    def delss():
        ...
        lab2.grid_forget(), nslb3.grid_forget(), nsbtn2.config(state=NORMAL), stateOfBtn(NORMAL)
    ...
    def det():
        ...
        if len(subName.get()) <= 18:
            if subName.get() not in n_s.values() and subName.get() != '':
                lab1.config(text=' ✔ ', fg=color["SUCCESS_COLOR"], bg=color["CHILD_COLOR"], font=12)
                a = True
            else:
                if subName.get() == '':
                    lab1.config(text='Subject Name cannot be empty.', fg=color["ERROR_COLOR"], bg=color["CHILD_COLOR"])
                else:
                    lab1.config(text=f'!! {subName.get()} is already used. ', fg=color["ERROR_COLOR"], font=(text["ALERT_FONT"], 14, 'bold'), bg=color["CHILD_COLOR"])
        else:
            lab1.config(text=' Subject Name cannot be greater than 18', fg=color["ERROR_COLOR"], bg=color["CHILD_COLOR"], font=(text["ALERT_FONT"], 14, 'bold'))

        if shortSub.get() not in n_s.keys() and shortSub.get() != '':
            lab2.config(text=' ✔ ', bg=color["CHILD_COLOR"], fg=color["SUCCESS_COLOR"], font=12)
            b = True
        else:
            if shortSub.get() == '':
                lab2.config(text='Shortcut Key cannot be empty.', fg=color["ERROR_COLOR"], bg=color["CHILD_COLOR"], font=(text["ALERT_FONT"], 14, 'bold'))
            else:
        ...
        if a and b:
            openDB(tableName, "INSERT INTO sub_log VALUES (?,?)", (shortSub.get(), subName.get()), True)
...
def add_one():
    ...
    if len(n_s) > 0:
        ...
        aolbl1.config(text=' These are the subjects: ', font=(text["HEADING_FONT"], 18, 'bold'), fg=color["TEXT_COLOR"], bg=color["CHILD_COLOR"])
        ...
        aolb2.config(text='  Subject  ', font=14, bg=color["CHILD_COLOR"], fg=color["TEXT_COLOR"])
        aolb3.config(text='  Topic  ', font=14, bg=color["CHILD_COLOR"], fg=color["TEXT_COLOR"])
        aolb4.config(text='  For how many minutes  ', font=14, bg=color["CHILD_COLOR"], fg=color["TEXT_COLOR"])
        ...
        def delstt():
            ...
            tp3.grid_forget(), aolb5.grid_forget(), b1.config(state=NORMAL), tp4.grid_forget(), stateOfBtn(NORMAL)
        ...
        def det():
            ...
            if sub.get() not in n_s.keys():
                tp2.config(text='* This Subject not found.', fg=color["ERROR_COLOR"], bg=color["CHILD_COLOR"], font=(text["ALERT_FONT"], 14, 'bold'))
            else:
                tp2.config(text='  ✔  ', bg=color["CHILD_COLOR"], fg=color["SUCCESS_COLOR"], font=20)
                da = True

            if time.get().isdigit() and time.get() != '':
                db = True
                tp3.config(text='  ✔  ', bg=color["CHILD_COLOR"], fg=color["SUCCESS_COLOR"], font=20)
            else:
                tp3.config(text=' * The time should be in integer format.', fg=color["ERROR_COLOR"], bg=color["CHILD_COLOR"], font=(text["ALERT_FONT"], 14, 'bold'))

            if len(topic.get()) > 50:
                tp4.config(text=' !! Topic is to large ', fg=color["ERROR_COLOR"], bg=color["CHILD_COLOR"], font=(text["ALERT_FONT"], 14, 'bold'))
            else:
                tp4.config(text='  ✔  ', bg=color["CHILD_COLOR"], fg=color["SUCCESS_COLOR"], font=20)
                dc = True

            if da and db and dc:
                timeStamp = datetime.now().strftime('%d-%m-%y: %H:%M:%S')
                openDB(tableName, "INSERT INTO log VALUES (?,?,?,?)", (timeStamp, n_s[sub.get()], topic.get(), time.get(),), True)
                aolb5.config(text='✔ Your data has been added Successfully.', font=14, bg=color["CHILD_COLOR"], fg=color["SUCCESS_COLOR"])
        ...
        tp2.config(text='   !!! No Subjects were found !!!   ', bg=color["CHILD_COLOR"], fg=color["ERROR_COLOR"], font=(text["ALERT_FONT"], 15, 'bold'))
        tp2.grid(row=0, column=0)
...
def show_data():
    ...
    if len(items) > 0:
        ...
        style.configure("mystyle.Treeview", font=(text["SUB_HEADING_FONT"], 15))

        style.theme_use("default")
        style.map('Treeview', background=[('selected', 'black')])
        style.map('Treeview.Heading', font=[(None, 16)])

        lab1.config(text="Table Data", font=(text["SUB_HEADING_FONT"], 20), justify='center', bg=color["CHILD_COLOR"], fg=color["TEXT_COLOR"])
        ...
        listBox.tag_configure('odd', background=color["STRIP_COL_ODD"], font=(text["TEXT_FONT"], 12))
        listBox.tag_configure('even', background=color["STRIP_COL_EVN"], font=(text["TEXT_FONT"], 12))
        count = 0
        ...
        aslb1.config(text='!!! No Data was found. !!!   ',  bg=color["CHILD_COLOR"], fg=color["ERROR_COLOR"], font=(text["ALERT_FONT"], 15, 'bold'))
        aslb1.grid(row=0, column=0)

    vdbtn.config(text='X'.center(3), command=close, bg='red')
    vdbtn.grid(row=0, column=3, sticky=NE, ipadx=10, pady=10, ipady=2) # row=0, column=3, padx=10, pady=10, ipady=5, ipadx=5
...
def activitySearch():
    ...
    if len(items) > 0:
        aslb1.config(text='\t\tActivity Search', font=(text["SUB_HEADING_FONT"], 20, 'bold'), justify='center', bg=color["CHILD_COLOR"], fg=color["TEXT_COLOR"])
        aslb2.config(text='', bg=color["CHILD_COLOR"])
        aslb3.config(text=' Enter the Date here (To get Today\'s Activity Enter nothing) ', font=(text["HEADING_FONT"], 12, 'bold'), bg=color["CHILD_COLOR"], fg=color["TEXT_COLOR"])
        ...
        aslb1.config(text='You need to Add Data To View Your Activity   ', font=(text["ALERT_FONT"], 15, 'bold'), bg=color["CHILD_COLOR"], fg=color["ERROR_COLOR"])
        aslb1.grid(row=0, column=0)
...
def UpdateSubject():
    ...
    for i in range(len(Rid)):
        openDB(tableName, "UPDATE sub_log SET rowid = (?) WHERE rowid = (?)", (i + 1, Rid[i]), True)
...
def updateTable():
    ...
    for i in range(len(Rowid)):
        openDB(tableName, "UPDATE log SET rowid=(?) WHERE rowid=(?)", (i + 1, Rowid[i]), True)
...
def DelSubRow():
    ...
    if len(n_s):
        ...
        tp2.config(text='Delete Subject/Row Data.'.rjust(10), font=(text["SUB_HEADING_FONT"], 20, 'bold'), justify='center', bg=color["CHILD_COLOR"], fg=color["TEXT_COLOR"])
        delsrlb.config(text='Do you want to delete the Subject(s) or Row(r) : ', font=(text['TEXT_FONT'], 14), fg=color["TEXT_COLOR"], bg=color["CHILD_COLOR"])
        ...
        def ok():
            if des.get() == 's':
                ...
                delslb.config(text='Select rowid of the subject you want to remove', font=8, bg=color["CHILD_COLOR"])
                ...
                delrid.config(text='\tRowid?? ', font=('Bahnschrift SemiLight', 12), bg=color["CHILD_COLOR"])
                delrid.grid(row=r + 1, column=0, stick=NW, pady=20)
                Id.grid(row=r + 1, column=0, pady=20)

                def OKK():
                    items = openDB(tableName, "SELECT sub_short FROM sub_log WHERE rowid = (?)", (Id.get(),))
                    n_s = subjects()
                    ...
                    try:
                        tp4.grid_forget()
                        s = n_s[items[0][0]]
                        a = openDB(tableName, "SELECT rowid,topics FROM log WHERE subject = (?)", (s,))
                        ...
                        if len([i[1] for i in a]) > 0:
                            ...
                            dellb2.config(text=f'{sub}', font=('Book Antiqua', 12), bg=color["CHILD_COLOR"])
                        ...
                        def close_grid():
                            dellb2.grid_forget();  dellb.grid_forget(); dely.grid_forget(); deln.grid_forget()
                            dellb3.grid_forget(); tp4.after(1000, ok); Id.delete(0, END)

                        def yes():
                            openDB(tableName, "DELETE from sub_log WHERE rowid = (?)", (str(Id.get()),), True)

                            for i in rid:
                                openDB(tableName, "DELETE from log WHERE rowid = (?)", (str(i),), True)

                            if temp2:
                                dellb3.config(text=f' {s} is removed. ', font=('Simsun', 15), bg=color["CHILD_COLOR"])
                            else:
                        ...
                        tp4.config(text=f'The Subject is not defined.', font=('Bahnschrift', 14, 'bold'), bg=color["CHILD_COLOR"])
                ...
                dellb2.config(text='Subject: '.rjust(15), font=3, bg=color["CHILD_COLOR"])
                ...
                def ok_row():
                    if subj.get() in n_s.keys():
                        ...
                        delslb.config(text=f' For {n_s[subj.get()]}', font=(text["TEXT_FONT"], 14), bg=color["CHILD_COLOR"])
                        delslb.grid(row=5, column=0, sticky=N + W)
                        sb2.config(command=delrlist4.yview)

                        a = openDB(tableName, "SELECT rowid,* FROM log WHERE subject = (?)", (n_s[subj.get()],))
                        ...
                        if len(a) > 0:
                            ...
                            delrlist4.insert(END, f'  !! No Topic entry was found !!')

                        delrlist4.grid(row=6, column=0, sticky=NW)

                        if forward:
                            items = a
                            delrid.config(text='Rowid?? '.rjust(15), font=('Bahnschrift SemiLight', 14), bg=color["CHILD_COLOR"])
                            delrid.grid(row=7, column=0, sticky=N + W, pady=15, padx=10)
                            Id.grid(row=7, column=0, pady=15)

                            def OOOk():
                                dellb4.config(text='')
                                if Id.get() in [str(i[0]) for i in items]:
                                    item = openDB(tableName, "SELECT rowid, topics, time FROM log WHERE rowid = (?)", (Id.get(),))[0]
                                    dellb3.config(text=f'Remove:  {item[1]} ?? ', font=6, bg=color["CHILD_COLOR"], fg='red')
                                    dellb3.grid(row=8, column=0, sticky=NW, pady=15)

                                    def del_grid():
                                        delrid.grid_forget(); Id.grid_forget(); dellb3.grid_forget()
                                        dely.grid_forget(); deln.grid_forget(); dellb4.grid_forget()
                                        delbtn4.grid_forget()
                                        delrlist4.delete('0', 'end')
                                        tp4.after(1000, ok_row)

                                    def yes():
                                        openDB(tableName, "DELETE from log WHERE rowid = (?)", (Id.get(),), True)
                                        openDB(tableName, "INSERT INTO delete_logs VALUES (?,?,?)", (subj.get(), item[1], item[2]), True)
...
def send_feedback(name, email_id, message):
    ...
    message = 'Name:_' + name + '<<::>>Email_Address:_' + email_id + '<<::>>Comment:_' + message
...
def feedback():
    ...
    Label(feedback_window, text=' FeedBack Form ', justify='center', font=(text['TEXT_FONT'], 20, 'bold'), bg=color["MENU_COLORS"]).grid(row=0, column=1)
    Label(feedback_window, text=' Name ', font=(text['TEXT_FONT'], 14), bg=color["MENU_COLORS"]).grid(row=1, column=0, padx=10, pady=10)
    Label(feedback_window, text=' Email Address ', font=(text['TEXT_FONT'], 14), bg=color["MENU_COLORS"]).grid(row=2, column=0, padx=10, pady=10)
    Label(feedback_window, text=' Comment ', font=(text['TEXT_FONT'], 14), bg=color["MENU_COLORS"]).grid(row=3, column=0, padx=10, pady=10)
...
f1 = Frame(root, bg=color["CHILD_COLOR"], borderwidth=3, width=700, height=600, relief=SUNKEN)
f1.grid(row=2, column=6, rowspan=50, padx=20, columnspan=3)

result_frame = Label(f1, text=f'This is the Result Area.', font=(text['MAIN_HEADING_FONT'], 14, 'bold'), borderwidth=2, width=70, height=30, relief=GROOVE, bg=color['CHILD_COLOR'])
result_frame.grid(row=2, column=3)

newSub = Button(root, text='Add a New Subject', command=newSubject, bg=color['MAIN_BTN_COLOR'])
inpData = Button(root, text='Inserting Data', command=add_one, bg=color['MAIN_BTN_COLOR'])
viewData = Button(root, text='To view the entire Data', command=show_data, bg=color['MAIN_BTN_COLOR'])
checkAct = Button(root, text='Check Activity', command=activitySearch, bg=color['MAIN_BTN_COLOR'])
delAct = Button(root, text='To Delete Subject or Row From the Data', command=DelSubRow, bg=color['MAIN_BTN_COLOR'])
disSub = Button(root, text='Display the Subjects', command=displaySubject, bg=color['MAIN_BTN_COLOR'])
...
if table_name != '':
    ...
    mainLabel1.config(text='You Need to select a table.', font=14, bg='#CCF381', fg='red')
    stateOfBtn(DISABLED)
...
comment = ''
tp2 = Label(f1); tp3 = Label(f1); tp4 = Label(f1)
lab1 = Label(f1); lab2 = Label(f1); lab3 = Label(f1)
delsrlb = Label(f1); dellb = Label(f1)
dellb2 = Label(f1); delslb = Label(f1); delrid = Label(f1); dellb4 = Label(f1); dellb3 = Label(f1)
aslb1 = Label(f1); aslb2 = Label(f1); aslb3 = Label(f1)
nslb1 = Label(f1); nslb2 = Label(f1); nslb3 = Label(f1)
aolbl1 = Label(f1); aolb2 = Label(f1); aolb3 = Label(f1); aolb4 = Label(f1); aolb5 = Label(f1)

delrlist4 = Listbox(f1); delrlist1 = Listbox(f1); delrlist3 = Listbox(f1)
myList = Listbox(f1); disSubList = Listbox(f1); addDList = Listbox(f1)
actSList = Listbox(f1); showDList = Listbox(f1)
...
f = 10
Id = Entry(f1, width=7, font=f); subj = Entry(f1, width=10, font=f); des = Entry(f1, width=30, font=f)
entered_date = Entry(f1, width=30, font=f); shortSub = Entry(f1, width=30, font=f)
subName = Entry(f1, width=30, font=f); sub = Entry(f1, width=30, font=f)
topic = Entry(f1, width=30, font=f); time = Entry(f1, width=30, font=f)

dely = Button(f1); deln = Button(f1); delrbtn = Button(f1); delbtn = Button(f1); delbtn4 = Button(f1); delsub = Button(f1)
delcl = Button(f1); asbtn = Button(f1); asbtn1 = Button(f1); vdbtn = Button(f1); disSubBtn = Button(f1)
nsbtn = Button(f1); nsbtn2 = Button(f1); b1 = Button(f1); aobtn1 = Button(f1)
...
root.mainloop()
>>>>>>> 0803ae3421cf4167e413459fc2c4375cfa21dc89
